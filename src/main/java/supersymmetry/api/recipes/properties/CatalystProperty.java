...
public class CatalystProperty extends RecipeProperty<CatalystPropertyValue> {
    ...
    public void drawInfo(@NotNull Minecraft minecraft, int x, int y, int color, Object value) {
        ...
        String localisedCatalystGroupName = I18n
                .format("gregtech.catalyst_group." + propertyValue.getCatalystGroup().getName() + ".name");
        if (tier == CatalystInfo.NO_TIER) {
            minecraft.fontRenderer.drawString(I18n.format("gregtech.recipe.catalyst", localisedCatalystGroupName, ""),
                    x, y, color);
        } else {
            minecraft.fontRenderer.drawString(
                    I18n.format("gregtech.recipe.catalyst", GTValues.VN[tier], localisedCatalystGroupName), x, y,
                    color);
        }
    }
}
