...
public class MetaTileEntityEvaporationPool extends RecipeMapMultiblockController {
     ...
     * "((IGregTechTileEntity)world.getTileEntity(pos)).getMetaTileEntity() instanceof IMultiblockAbilityPart"
     * is the way ceu gets mte from te. You might also try using MetaTileEntityHolder.
     */

    public static final int MAX_SQUARE_SIDE_LENGTH = 12; // two edge layers on either side, shouldn't exceed chunk
                                                         // boundary at max size
    ...
    int exposedBlocks = 0;
    byte[] wasExposed; // indexed with row*col + col with row = 0 being furthest and col 0 being leftmost when looking
                       // at controller
    ...
    static {
        ...
        validContainerStates.addAll(MetaBlocks.WIRE_COIL.getBlockState().getValidStates()); // add all coils as valid
                                                                                            // container blocks
    }
    ...
    public boolean updateStructureDimensions() {
        ...
        // distance to use when located edges of structure, moved inwards to container block portion for detection
        // purposes
        BlockPos.MutableBlockPos lPos = new BlockPos.MutableBlockPos(getPos().offset(back, 2)); // start in container
                                                                                                // section
        BlockPos.MutableBlockPos rPos = new BlockPos.MutableBlockPos(getPos().offset(back, 2)); // start in container
                                                                                                // section
        BlockPos.MutableBlockPos bPos = new BlockPos.MutableBlockPos(getPos().offset(back)); // start one before
                                                                                             // container section
        ...
        // if l or r dist exceed max, or if bdist exceeds max/ is 0, or if l and r dist individually don't exceed max,
        // but produce a columnCount > max
    ...
    public boolean isContainerBlock(@Nonnull World world, @Nonnull BlockPos.MutableBlockPos pos,
    ...
    public static String repeat(String s, int count) {
        ...
        // create empty char array, convert to string which places null terminators in all its positions, then replace
        // all
        ...
        for (int i = 0; i < rowCount; ++i) {
            ...
            for (int j = 2; j < columnCount + 2; ++j) {
                ...
                if ((j & 1) == 0) {
                    ...
                    // if on first row then use "C" [1], otherwise use "G" [0] (0 is only number which is non neg after
                    // * -1. this means bit flip keeps most sig as 1)
    ...
    protected BlockPattern createStructurePattern() {
        ...
        if (structurePattern != null && currColCount == columnCount && currRowCount == rowCount &&
                currContPos == controllerPosition)
        ...
        pattern = FactoryBlockPattern.start().aisle(repeat("E", columnCount + 4), repeat(" ", columnCount + 4))
                .aisle(repeat("E", columnCount + 4), " ".concat(repeat("E", columnCount + 2)).concat(" "));

        // place all generated aisles (rows stored closer to further, this wants them further to closer) and save to
        // pattern explicitly (unsure if the explicit assignment is necessary; probably no harm in being safe)
        for (int i = 0; i < rowCount; ++i) {
            pattern = pattern.aisle(containerRows[rowCount - 1 - i],
                    " E".concat(repeat("#", columnCount).concat("E ")));
        }

        // place last two aisles
        pattern = pattern.aisle(repeat("E", columnCount + 4), " ".concat(repeat("E", columnCount + 2)).concat(" "))
                .aisle(repeat("E", controllerPosition + 2).concat("S")
                        .concat(repeat("E", columnCount + 1 - controllerPosition)), repeat(" ", columnCount + 4))

                // begin predicates
                .where('S', selfPredicate())
                .where('E',
                        isEdge().or(autoAbilities(false, false, true, true, true, true, false).setMaxGlobalLimited(8))
                                .or(abilities(MultiblockAbility.INPUT_ENERGY).setMinGlobalLimited(0)
                                        .setMaxGlobalLimited(2).setPreviewCount(0)))
                .where('G', isGround()).where('C', isContainer()).where('#', air()).where(' ', any());
    ...
    protected TraceabilityPredicate isEdge() {
        ...
        Supplier<BlockInfo[]> supplier = () -> new BlockInfo[] { new BlockInfo(MetaBlocks.STONE_BLOCKS
                .get(StoneVariantBlock.StoneVariant.SMOOTH).getState(StoneVariantBlock.StoneType.CONCRETE_LIGHT)) };

        // returns true if blockstate is concrete
        return new TraceabilityPredicate(blockWorldState -> {
            IBlockState state = blockWorldState.getBlockState();
            return state == MetaBlocks.STONE_BLOCKS.get(StoneVariantBlock.StoneVariant.SMOOTH)
    ...
    protected TraceabilityPredicate isGround() {
        ...
        Supplier<BlockInfo[]> supplier = () -> new BlockInfo[] {
                new BlockInfo(SuSyBlocks.EVAPORATION_BED.getState(BlockEvaporationBed.EvaporationBedType.DIRT)) };
    ...
    // ensures coil pattern is either entirely there or completely absent for structure to be valid. Sets isHeated
    // accordingly
    ...
    // returns -1 for no errors, -2 for groundcheck error, else it returns position of failure, counting from left to
    // right then closer to further
    public int coilPatternCheck(boolean checkGround) {
        // first coil block has been found; ensure full coil pattern is present so that future validation checks may be
        // skipped
        String[] centerPattern = centerRowsPattern();
        IBlockState targetState = (coilStateMeta == -1) ? null : MetaBlocks.WIRE_COIL.getStateFromMeta(coilStateMeta); // if
                                                                                                                       // stored
                                                                                                                       // coilstate
                                                                                                                       // exists
        ...
        BlockPos.MutableBlockPos targetPos = new BlockPos.MutableBlockPos(getPos().offset(back, 2)); // place on first
                                                                                                     // row
        targetPos.move(right.getOpposite(), controllerPosition + 1); // move to the furthest left center block +1
                                                                     // (looking at controller) to account for immediate
                                                                     // right deviation in j loop
        ...
        for (int i = 0; i < rowCount; ++i) {
            ...
            for (int j = 2; j < columnCount + 2; ++j) {
                ...
                if (centerPattern[i].charAt(j) != 'C') {
                    ...
                    if (getWorld().getBlockState(targetPos) ==
                            SuSyBlocks.EVAPORATION_BED.getState(BlockEvaporationBed.EvaporationBedType.DIRT))
                        continue;
                    return ((i * columnCount + j - 2) * -1) - 2; // ground check failed; return index number's negative
                                                                 // shifted down two such that range => [0, INT_MAX] ->
                                                                 // [-INT_MAX, -2]
                }

                // store state of first coil. Might do weird stuff if meta of coilState > 7, as jei error-ed when trying
                // to display them even though they are same material type
                if (targetState == null) {
                    ...
                    continue; // proceed only if current coil is exact same as previous coil(s). equals seems to fail,
                              // so a string comparison works best)

                // current coilpos block is either not a coil (first coil position), or it is not identical to previous
                // coil(s)
                return i * columnCount + j - 2; // indicate failure of solid coil structure through >0 value. Stores
                                                // relative position of failure in int (kind of pointless now; was for
                                                // old code that got pos from this)
            }

            targetPos.move(back); // go back one row
            targetPos.move(right.getOpposite(), columnCount); // reset to leftmost center position +1 unit left due to
                                                              // order of traversal inside nested loop
        }

        if (!checkGround)
            coilStateMeta = targetState.getBlock().getMetaFromState(targetState); // store correct coilstate (necessary
                                                                                  // as passing coilState would pass the
                                                                                  // reference by value, meaning
                                                                                  // reassignment would not carry to
                                                                                  // caller)
        return -1; // no checks failed
    }

    // takes result int from coilPatternCheck, outputting boolean related to whether or not structure was found to be
    // valid
    ...
    /*
     * apparently the structurepattern's worldstate is being set to error at the furthest and leftmost coil block
     * whenever
     * multiple coils are destroyed, which causes weird behavior when attempting to reform the structure. The only way
     * to get around
     * this without more thoroughly understanding how the structure checks work is to manually change the error to any
     * invalid center block,
    ...
    public void checkStructurePattern() {
        // This is here so that it automatically updates the dimensions once a second if it isn't formed [this method is
    ...
    public void addInformation(ItemStack stack, World player, @NotNull List<String> tooltip, boolean advanced) {
        tooltip.add(I18n.format("gregtech.machine.evaporation_pool.tooltip.info", MAX_SQUARE_SIDE_LENGTH,
                MAX_SQUARE_SIDE_LENGTH));
        if (TooltipHelper.isShiftDown()) {
            tooltip.add(I18n.format("gregtech.machine.evaporation_pool.tooltip.structure_info", MAX_SQUARE_SIDE_LENGTH,
                    MAX_SQUARE_SIDE_LENGTH) + "\n");
        }
    ...
    public BlockPos.MutableBlockPos getCorner(boolean isClose, boolean isLeft) {
        ...
        if (isLeft) {
            ...
            corner.move(left.getOpposite(), columnCount - controllerPosition - 1); // line up w/ furthest right interior
                                                                                   // column
        }
     ...
     * return;
     * 
     * Cuboid6 partialFluidBox = new Cuboid6(1.0625 / 16.0, 2.0625 / 16.0, 1.0625 / 16.0, 14.9375 / 16.0, 14.9375 /
     * 16.0, 14.9375 / 16.0);
     * 
     ...
     * Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(fluidStill.toString());
    ...
    public <T extends MultiblockRecipeLogic> void renderFluid(CCRenderState renderState, Matrix4 translation,
                                                              IVertexOperation[] pipeline, IBlockAccess world,
                                                              EnumFacing frontFacing, T recipeMapWorkable,
                                                              BlockPos.MutableBlockPos controllerPos) {
        if ((this.tickTimer + 1) % 20 == 0) {
            SusyLog.logger.atError().log("About to attempt rendering with getWorld: " +
                    (world == null ? "null" : world) + ", isStructureFormed: " + (isStructureFormed()) +
                    ", isActive: " +
                    (recipeMapWorkable == null ? "null" : (recipeMapWorkable.isActive()) + ", previousRecipe: " +
                            (recipeMapWorkable.getPreviousRecipe() == null ? "null" : recipeMapWorkable.isActive())));
        } else {
            return;
        }
        ...
        if (controllerPos == null || !this.isStructureFormed() || !recipeMapWorkable.isActive() ||
                recipeMapWorkable.getPreviousRecipe() == null) {
            return;
        }

        SusyLog.logger.atError().log("Passed rendering check");

        // Create a box with slight margins since going from exactly 0 to exactly 1 causes some faces to not render
        // properly
        ...
        // apply translations to scale properly; "back distance and length are switched in the case of WEST and EAST as
        // a form of "rotation" since translation.rotate is broken..."
        translation.translate((corner.getX() - controllerPos.getX()), 1, corner.getZ() - controllerPos.getZ());
        translation.scale(EWFacing * rowCount + NSFacing * columnCount, 1,
                EWFacing * columnCount + NSFacing * rowCount);
     ...
     * if ((tickTimer +1) % 20 == 0) {
     * SusyLog.logger.atError().log("BEFORE RENDER CALL with world: " + (this.getWorld() == null ? "null" :
     * this.getWorld()) + ", isStructureFormed: " + (this.isStructureFormed()) + ", isActive: " +
     * (this.getRecipeMapWorkable() == null ? "null" : (this.getRecipeMapWorkable().isActive()) + ", previousRecipe: " +
     * (this.getRecipeMapWorkable().getPreviousRecipe() == null ? "null" : this.getRecipeMapWorkable().isActive())));
     ...
     * renderFluid(renderState, translation, pipeline, getWorld(), getFrontFacing(), getRecipeMapWorkable(), new
    ...
    public void update() {
        ...
        // apparently causes significant lag; removal of this call will result in rendering issues due to super update
        // concluding coils should be on innappropriately
        // setCoilActivity(false); // solves world issue reload where coils would be active even if multi was not
        // running
        ...
        // should skip/cost an extra tick the first time and then anywhere from 1-9 extra when rolling over. Determines
        // exposedblocks
        if (tickTimer % 10 == 0 && tickTimer != 0) {
            // no sunlight heat generated when raining or during night. May be incongruent with partial exposure to sun,
            // but oh well
            if (getWorld().isRainingAt(getPos().offset(getFrontFacing().getOpposite(), 2)) || !getWorld().isDaytime()) {
                exposedBlocks = 0;
                for (int i = 0; i < wasExposed.length; ++i) wasExposed[i] = 0; // do not leave discrepancy between
                                                                               // exposedBlock count and wasExposed
            }
            // checks for ow and skylight access to prevent beneath portal issues (-1 = the Nether, 0 = normal world)
                ...
                // tickTimer is always multiple of 20 at this point, so division by 20 yields proper counter. You can
                // treat (tickTimer/20) as 'i'
                int row = ((tickTimer / 20) / columnCount) % rowCount; // going left to right, further to closer
                                                                       // checking skylight access.
                int col = ((tickTimer / 20) % columnCount);
                // places blockpos for skycheck into correct position. Row counts from furthest to closest (kinda
                ...
                if (wasExposed == null || wasExposed.length != rowCount * columnCount) {
                    ...
                    exposedBlocks = 0;
                    SusyLog.logger.atError().log("Detected evaporation pool with invalid wasExposed array at pos: " +
                            this.getPos() + "; setting explosed blocks to 0");
                }

                // Perform skylight check
                if (!getWorld().canBlockSeeSky(skyCheckPos)) {
                    // only decrement exposedBlocks if previously exposed block is found to no longer be exposed and one
                    // full pass has occurred
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        MultiblockDisplayText.builder(textList, isStructureFormed())
                ...
                .addEnergyUsageLine(getEnergyContainer()).addCustom(tl -> {
                    // coil coefficient
                    if (isStructureFormed()) {
                        // handle heating contributions
                        if (isHeated()) {
                            tl.add(TextComponentUtil
                                    .translationWithColor(TextFormatting.WHITE,
                                            "gregtech.top.evaporation_pool_heated_preface")
                                    .appendText(" ").appendSibling(TextComponentUtil.translationWithColor(
                                            TextFormatting.GREEN, "gregtech.top.evaporation_pool_is_heated")));
                            ...
                            tl.add(TextComponentUtil
                                    .translationWithColor(TextFormatting.WHITE,
                                            "gregtech.top.evaporation_pool_heated_preface")
                                    .appendText(" ").appendSibling(TextComponentUtil.translationWithColor(
                                            TextFormatting.RED, "gregtech.top.evaporation_pool_not_heated")));
                        }
                        ...
                        tl.add(TextComponentUtil
                                ...
                                .appendText(" ").appendSibling(TextComponentUtil.translationWithColor(
                                        TextFormatting.GREEN, TextFormattingUtil.formatNumbers(exposedBlocks))));
                        ...
                        tl.add(TextComponentUtil
                                ...
                                .appendText(" ").appendSibling(
                                        TextComponentUtil
                                                .stringWithColor(TextFormatting.YELLOW,
                                                        TextFormattingUtil.formatNumbers(this.getKiloJoules()))
                                                .appendText(".")
                                                .appendSibling(TextComponentUtil.stringWithColor(TextFormatting.YELLOW,
                                                        EvaporationPoolInfoProvider
                                                                .constLengthToString(this.getJoulesBuffer())))
                                                .appendText(" ").appendSibling(
                                                        TextComponentUtil.translationWithColor(TextFormatting.WHITE,
                                                                "gregtech.top.evaporation_pool.kilojoules"))));

                        tl.add(TextComponentUtil
                                .translationWithColor(
                                        TextFormatting.WHITE, "gregtech.mutliblock.evaporation_pool.rolling_average")
                                .appendText(" ")
                                .appendSibling(TextComponentUtil.stringWithColor(TextFormatting.YELLOW,
                                        TextFormattingUtil.formatNumbers(getRollingAverageJt())))
                                .appendText(" ").appendSibling(TextComponentUtil.translationWithColor(
                                        TextFormatting.WHITE, "gregtech.multiblock.evaporation_pool.joules_per_tick")));

                        tl.add(TextComponentUtil
                                .translationWithColor(
                                        TextFormatting.WHITE, "gregtech.multiblock.evaporation_pool.average_speed")
                                .appendText(" ")
                                .appendSibling(TextComponentUtil.stringWithColor(TextFormatting.GREEN,
                                        getAverageRecipeSpeedString()))
                                .appendSibling(TextComponentUtil.stringWithColor(TextFormatting.WHITE, "x"))); // add
                                                                                                               // empty
                                                                                                               // space
                                                                                                               // to
                                                                                                               // visually
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound data) {
        ...
        data.setByteArray("wasExposed",
                this.wasExposed == null ? new byte[this.rowCount * this.columnCount] : this.wasExposed);
    ...
    private void evaporationParticles() {
        ...
        // conversion from blockpos to in world pos places particle position at corner of block such that relative
        // direction must be accounted for
        // add 1 if x pos or z pos (offsets mutually exclusively non-zero for given facing) as conversion to float pos
        // rounds down in both coords
        ...
        for (int i = 0; i < Math.max(1, columnCount * rowCount / 3); i++) {
            // either single line along x axis intersects all rows, or it intersects all columns. Same applies to z
            // axis. getXOffset indicates +, -, or "0" direction of facing for given axis
            float xLength = (back.getXOffset() * rowCount) + (left.getXOffset() * columnCount * -1); // we start on
                                                                                                     // leftmost closest
                                                                                                     // corner and want
                                                                                                     // to go back and
                                                                                                     // to the right
            float zLength = (back.getZOffset() * rowCount) + (left.getZOffset() * columnCount * -1); // so we invert the
                                                                                                     // sign of the left
                                                                                                     // offsets to
                                                                                                     // effectively get
                                                                                                     // right
            ...
            // if (tickTimer % 100 == 0) SusyLog.logger.atError().log("xLength: " + xLength + ", zLength: " + zLength +
            // ", leftOffset: " + leftOffset + ", backOffset: " + backOffset + ", pos: " + pos + ", controller pos: " +
            // getPos());

            float xPos = pos.getX() + (xLength * GTValues.RNG.nextFloat()); // scale x length by random amount to get
                                                                            // output coord
            float yPos = pos.getY() + 0.75F; // shit out particles one quarter of a block below the surface of the
                                             // interior to give effect of gases rising from bottom
    ...
    public int calcMaxSteps(int jStepSize) {
        int stepCount = (getKiloJoules() * 1000) / jStepSize; // max number of times jStepSize can cleanly be deducted
                                                              // from kiloJoules
        int remainder = (stepCount + 1) * jStepSize - getKiloJoules() * 1000; // remaining joules needed to not waste
                                                                              // partial kJ

        if (joulesBuffer >= remainder) ++stepCount;
        else remainder = 0;

        stepCount += (joulesBuffer - remainder) / jStepSize; // number of jSteps which can come entirely from
                                                             // joulesBuffer
        return stepCount;
    }
